...
def bill(name, phno, paym, items, purch, prices, saved, taxamt, ship, payamt, mode, addr):
    ...
    file.write("\n=============================================================")
    file.write("\n\t\tITEM\tQUANTITY\tPRICE\tSAVED\tTOTAL")                     
    file.write("\n-------------------------------------------------------------")

    for key in purch.keys():
        if key=="A":
            file.write(str("\n\t\t"+str(key)+"\t\t "+str(purch['A'])+" \t\t "+str(items['A'][0])+" \t\t "+str(saved['A'])+" \t\t "+str(prices['A'])))
        elif key=='B':
